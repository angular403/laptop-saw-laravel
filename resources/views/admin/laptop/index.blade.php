...
@section('title')
    Data Handphone | Sitem Rekomendasi Handhpone
@endsection
@section('content')
<br>
<div class="row">
    <div class="col-12">
        <div class="card-box">
            ...
            <h4 class="header-title m-t-0">Tambah Handphone</h4>
...
<div class="row">
    <div class="col-12">
        <div class="card-box table-responsive">
            <h4 class="m-t-0 header-title"><b>Daftar Handhpone</b></h4>
...
@push('scripts')
        <script type="text/javascript">
            const EURO_TO_RUPIAH = parseFloat({{env('EURO_TO_RUPIAH')}});
            function lihatLaptop(trigerer){
                    ...
                    const harga_rupiah = $("#table-mahasiswa").DataTable().row(tr).data().Price_euros*EURO_TO_RUPIAH
                    $(modal + " #price_rupiah").html("Harga Rupiah : "+harga_rupiah.toLocaleString('id-ID'));

                    $(modal+" #spesifikasi_umum").html("Spesifikasi Umum: "+$("#table-mahasiswa").DataTable().row(tr).data().SpesifikasiUmum)
                    const gambar = $("#table-mahasiswa").DataTable().row(tr).data().Gambar;
                    console.log(gambar,$("#table-mahasiswa").DataTable().row(tr).data())
                    if(gambar){
                        $(modal+" #gambar").html(`<div>Gambar</div><img src="{{url('')}}/storage/${gambar}" style="max-width:100px;">`)
                    }else{
                        $(modal+" #gambar").html('')
                    }
                }
